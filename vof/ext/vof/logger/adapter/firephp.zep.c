...
ZEPHIR_INIT_CLASS(Vof_Logger_Adapter_Firephp) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Vof\\Logger\\Adapter, Firephp, vof, logger_adapter_firephp, vof_logger_adapter_ce, vof_logger_adapter_firephp_method_entry, 0);

	zend_declare_property_bool(vof_logger_adapter_firephp_ce, SL("_initialized"), 0, ZEND_ACC_PRIVATE TSRMLS_CC);

	zend_declare_property_long(vof_logger_adapter_firephp_ce, SL("_index"), 1, ZEND_ACC_PRIVATE TSRMLS_CC);
...
PHP_METHOD(Vof_Logger_Adapter_Firephp, getFormatter) {
	...
	if (Z_TYPE_P(&_0) != IS_OBJECT) {
		...
		object_init_ex(&_1$$3, vof_logger_formatter_firephp_ce);
...
PHP_METHOD(Vof_Logger_Adapter_Firephp, logInternal) {
	...
	if (!(zephir_is_true(&_0))) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "header", &_2, 41, &_1$$3);
		zephir_check_call_status();
	...
	zephir_check_call_status();
	ZVAL_LONG(&_4, 4500);
	ZEPHIR_CALL_FUNCTION(&chunk, "str_split", NULL, 42, &format, &_4);
	zephir_check_call_status();
	ZEPHIR_OBS_VAR(&index);
	zephir_read_property(&index, this_ptr, SL("_index"), PH_NOISY_CC);
	zephir_is_iterable(&chunk, 0, "vof/logger/adapter/firephp.zep", 92);
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&chunk), _7, _8, _6)
	{
		...
		ZEPHIR_CALL_FUNCTION(NULL, "header", &_2, 41, &content);
		zephir_check_call_status();
