 ...
 * </code>
 */
ZEPHIR_INIT_CLASS(Vof_Logger_Adapter_Stream) {

	ZEPHIR_REGISTER_CLASS_EX(Vof\\Logger\\Adapter, Stream, vof, logger_adapter_stream, vof_logger_adapter_ce, vof_logger_adapter_stream_method_entry, 0);
	 ...
	 * @var resource
	 */
	zend_declare_property_null(vof_logger_adapter_stream_ce, SL("_stream"), ZEND_ACC_PROTECTED TSRMLS_CC);
 ...
 * @param array options
 */
PHP_METHOD(Vof_Logger_Adapter_Stream, __construct) {
	...
	if (zephir_array_isset_string_fetch(&mode, options, SL("mode"), 0)) {
		if (zephir_memnstr_str(&mode, SL("r"), "vof/logger/adapter/stream.zep", 64)) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_logger_exception_ce, "Stream must be opened in append or write mode", "vof/logger/adapter/stream.zep", 65);
			return;
		}
	} else {
	...
	ZEPHIR_CALL_FUNCTION(&stream, "fopen", NULL, 39, &name, &mode);
	zephir_check_call_status();
	if (!(zephir_is_true(&stream))) {
		ZEPHIR_INIT_VAR(&_0$$6);
		object_init_ex(&_0$$6, vof_logger_exception_ce);
		ZEPHIR_INIT_VAR(&_1$$6);
		ZEPHIR_CONCAT_SVS(&_1$$6, "Can't open stream '", &name, "'");
		ZEPHIR_CALL_METHOD(NULL, &_0$$6, "__construct", NULL, 19, &_1$$6);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_0$$6, "vof/logger/adapter/stream.zep", 76 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
 ...
 */
PHP_METHOD(Vof_Logger_Adapter_Stream, getFormatter) {
 ...
 */
PHP_METHOD(Vof_Logger_Adapter_Stream, logInternal) {
	...
	if (Z_TYPE_P(&stream) != IS_RESOURCE) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_logger_exception_ce, "Cannot send message to the log because it is invalid", "vof/logger/adapter/stream.zep", 103);
		return;
	}
 ...
 * Closes the logger
 */
PHP_METHOD(Vof_Logger_Adapter_Stream, close) {
