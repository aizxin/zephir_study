...
ZEPHIR_INIT_CLASS(Vof_Logger_Adapter) {
	...
	ZEPHIR_REGISTER_CLASS(Vof\\Logger, Adapter, vof, logger_adapter, vof_logger_adapter_method_entry, ZEND_ACC_EXPLICIT_ABSTRACT_CLASS);
	 ...
	 * @var boolean
	 */
	zend_declare_property_bool(vof_logger_adapter_ce, SL("_transaction"), 0, ZEND_ACC_PROTECTED TSRMLS_CC);
	 ...
	 * @var array
	 */
	zend_declare_property_null(vof_logger_adapter_ce, SL("_queue"), ZEND_ACC_PROTECTED TSRMLS_CC);
	 ...
	 * @var object
	 */
	zend_declare_property_null(vof_logger_adapter_ce, SL("_formatter"), ZEND_ACC_PROTECTED TSRMLS_CC);
	 ...
	 * @var int
	 */
	zend_declare_property_long(vof_logger_adapter_ce, SL("_logLevel"), 9, ZEND_ACC_PROTECTED TSRMLS_CC);

	vof_logger_adapter_ce->create_object = zephir_init_properties_Vof_Logger_Adapter;

	zend_class_implements(vof_logger_adapter_ce TSRMLS_CC, 1, vof_logger_adapterinterface_ce);
	return SUCCESS;
...
PHP_METHOD(Vof_Logger_Adapter, commit) {
	...
	if (!(zephir_is_true(&_0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_logger_exception_ce, "There is no active transaction", "vof/logger/adapter.zep", 107);
		return;
	}
	if (0) {
		zephir_update_property_zval(this_ptr, SL("_transaction"), &__$true);
	...
	zephir_is_iterable(&_1, 0, "vof/logger/adapter.zep", 125);
...
PHP_METHOD(Vof_Logger_Adapter, rollback) {
	...
	if (!(zephir_is_true(&transaction))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_logger_exception_ce, "There is no active transaction", "vof/logger/adapter.zep", 139);
		return;
	}
	if (0) {
		zephir_update_property_zval(this_ptr, SL("_transaction"), &__$true);
...
PHP_METHOD(Vof_Logger_Adapter, log) {
	...
	if (ZEPHIR_GE(&_2, &toggledType)) {
		...
		if (zephir_is_true(&_3$$8)) {
			...
			if (!_5$$9) {
			_5$$9 = zephir_fetch_class_str_ex(SL("Vof\\Logger\\Item"), ZEND_FETCH_CLASS_AUTO);
			}
			object_init_ex(&_4$$9, _5$$9);
			if (zephir_has_constructor(&_4$$9 TSRMLS_CC)) {
				ZEPHIR_CALL_METHOD(NULL, &_4$$9, "__construct", NULL, 0, &toggledMessage, &toggledType, &timestamp, &context);
				zephir_check_call_status();
			}
