...
ZEPHIR_INIT_CLASS(Vof_Logger_Formatter_Line) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Vof\\Logger\\Formatter, Line, vof, logger_formatter_line, vof_logger_formatter_ce, vof_logger_formatter_line_method_entry, 0);
	 ...
	 * @var string
	 */
	zend_declare_property_string(vof_logger_formatter_line_ce, SL("_dateFormat"), "D, d M y H:i:s O", ZEND_ACC_PROTECTED TSRMLS_CC);
	 ...
	 * @var string
	 */
	zend_declare_property_string(vof_logger_formatter_line_ce, SL("_format"), "[%date%][%type%] %message%", ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Vof_Logger_Formatter_Line, format) {
	...
	if (zephir_memnstr_str(&format, SL("%date%"), "vof/logger/formatter/line.zep", 80)) {
		ZEPHIR_INIT_VAR(&_0$$3);
		zephir_read_property(&_1$$3, this_ptr, SL("_dateFormat"), PH_NOISY_CC | PH_READONLY);
		ZVAL_LONG(&_2$$3, timestamp);
		ZEPHIR_CALL_FUNCTION(&_3$$3, "date", NULL, 48, &_1$$3, &_2$$3);
	...
	if (zephir_memnstr_str(&format, SL("%type%"), "vof/logger/formatter/line.zep", 87)) {
