...
ZEPHIR_INIT_CLASS(Vof_Session_Adapter_Redis) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Vof\\Session\\Adapter, Redis, vof, session_adapter_redis, vof_session_adapter_ce, vof_session_adapter_redis_method_entry, 0);

	zend_declare_property_null(vof_session_adapter_redis_ce, SL("_redis"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_long(vof_session_adapter_redis_ce, SL("_lifetime"), 8600, ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Vof_Session_Adapter_Redis, __construct) {
	...
	ZEPHIR_INIT_VAR(&_2);
	object_init_ex(&_2, vof_cache_backend_redis_ce);
	ZEPHIR_INIT_VAR(&_3);
	object_init_ex(&_3, vof_cache_frontend_none_ce);
	...
	ZEPHIR_CALL_METHOD(NULL, &_2, "__construct", NULL, 81, &_3, &options);
	zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(NULL, "session_set_save_handler", NULL, 79, &_6, &_8, &_9, &_10, &_11, &_12);
	zephir_check_call_status();
	ZEPHIR_CALL_PARENT(NULL, vof_session_adapter_redis_ce, getThis(), "__construct", &_13, 0, &options);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
