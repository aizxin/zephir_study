 ...
 *</code>
 */
ZEPHIR_INIT_CLASS(Vof_Cache_Backend_Libmemcached) {

	ZEPHIR_REGISTER_CLASS_EX(Vof\\Cache\\Backend, Libmemcached, vof, cache_backend_libmemcached, vof_cache_backend_ce, vof_cache_backend_libmemcached_method_entry, 0);

	zend_declare_property_null(vof_cache_backend_libmemcached_ce, SL("_memcache"), ZEND_ACC_PROTECTED TSRMLS_CC);
 ...
 * @param	array options
 */
PHP_METHOD(Vof_Cache_Backend_Libmemcached, __construct) {
	...
	ZEPHIR_CALL_PARENT(NULL, vof_cache_backend_libmemcached_ce, getThis(), "__construct", &_2, 0, frontend, options);
 ...
 */
PHP_METHOD(Vof_Cache_Backend_Libmemcached, _connect) {
	...
	if (ZEPHIR_IS_EMPTY(&_0)) {
		...
		if (!(zephir_array_isset_string_fetch(&servers, &options, SL("servers"), 0))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Servers must be an array", "vof/cache/backend/libmemcached.zep", 120);
			return;
		}
		if (Z_TYPE_P(&servers) != IS_ARRAY) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Servers must be an array", "vof/cache/backend/libmemcached.zep", 124);
			return;
		}
		...
		if (Z_TYPE_P(&client) != IS_ARRAY) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Client options must be instance of array", "vof/cache/backend/libmemcached.zep", 132);
			return;
		}
		ZEPHIR_CALL_METHOD(&_1$$4, &memcache, "setoptions", NULL, 0, &client);
		zephir_check_call_status();
		if (!(zephir_is_true(&_1$$4))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cannot set to Memcached options", "vof/cache/backend/libmemcached.zep", 136);
			return;
		}
		ZEPHIR_CALL_METHOD(&_2$$4, &memcache, "addservers", NULL, 0, &servers);
		zephir_check_call_status();
		if (!(zephir_is_true(&_2$$4))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cannot connect to Memcached server", "vof/cache/backend/libmemcached.zep", 140);
			return;
		}
	}
 ...
 */
PHP_METHOD(Vof_Cache_Backend_Libmemcached, get) {
 ...
 */
PHP_METHOD(Vof_Cache_Backend_Libmemcached, save) {
	...
	if (!(zephir_is_true(&lastKey))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cache must be started first", "vof/cache/backend/libmemcached.zep", 196);
		return;
	}
	...
	if (!(zephir_is_true(&success))) {
		...
		object_init_ex(&_1$$15, vof_cache_exception_ce);
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$15, "__construct", NULL, 19, &_3$$15);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$15, "vof/cache/backend/libmemcached.zep", 240 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_OBS_VAR(&options);
	...
	if (!(zephir_array_isset_string_fetch(&specialKey, &options, SL("statsKey"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/libmemcached.zep", 246);
		return;
	}
 ...
 * @return boolean
 */
PHP_METHOD(Vof_Cache_Backend_Libmemcached, delete) {
	...
	ZEPHIR_OBS_VAR(&specialKey);
	if (!(zephir_array_isset_string_fetch(&specialKey, &options, SL("statsKey"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/libmemcached.zep", 299);
		return;
	}
	if (!ZEPHIR_IS_STRING(&specialKey, "")) {
		ZEPHIR_CALL_METHOD(&keys, &memcache, "get", NULL, 0, &specialKey);
 ...
 * </code>
 */
PHP_METHOD(Vof_Cache_Backend_Libmemcached, queryKeys) {
	...
	ZEPHIR_OBS_VAR(&specialKey);
	if (!(zephir_array_isset_string_fetch(&specialKey, &options, SL("statsKey"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/libmemcached.zep", 341);
		return;
	}
	if (ZEPHIR_IS_STRING(&specialKey, "")) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cached keys need to be enabled to use this function (options['statsKey'] == '_PHCM')!", "vof/cache/backend/libmemcached.zep", 345);
		return;
	}
	ZEPHIR_CALL_METHOD(&keys, &memcache, "get", NULL, 0, &specialKey);
	zephir_check_call_status();
	...
	zephir_is_iterable(&keys, 1, "vof/cache/backend/libmemcached.zep", 363);
 ...
 * @param int lifetime
 */
PHP_METHOD(Vof_Cache_Backend_Libmemcached, exists) {
 ...
 * @param string keyName
 */
PHP_METHOD(Vof_Cache_Backend_Libmemcached, increment) {
 ...
 * @param string keyName
 */
PHP_METHOD(Vof_Cache_Backend_Libmemcached, decrement) {
 ...
 *</code>
 */
PHP_METHOD(Vof_Cache_Backend_Libmemcached, flush) {
	...
	ZEPHIR_OBS_VAR(&specialKey);
	if (!(zephir_array_isset_string_fetch(&specialKey, &options, SL("statsKey"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/libmemcached.zep", 490);
		return;
	}
	if (ZEPHIR_IS_STRING(&specialKey, "")) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cached keys need to be enabled to use this function (options['statsKey'] == '_PHCM')!", "vof/cache/backend/libmemcached.zep", 494);
		return;
	}
	ZEPHIR_CALL_METHOD(&keys, &memcache, "get", NULL, 0, &specialKey);
	zephir_check_call_status();
	...
	ZEPHIR_INIT_VAR(&_0);
	zephir_is_iterable(&keys, 0, "vof/cache/backend/libmemcached.zep", 509);
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&keys), _2, _3, _1)
	{
		ZEPHIR_INIT_NVAR(&key);
		if (_3 != NULL) { 
