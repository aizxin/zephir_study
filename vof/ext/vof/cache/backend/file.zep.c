...
ZEPHIR_INIT_CLASS(Vof_Cache_Backend_File) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Vof\\Cache\\Backend, File, vof, cache_backend_file, vof_cache_backend_ce, vof_cache_backend_file_method_entry, 0);
	 ...
	 * @var boolean
	 */
	zend_declare_property_bool(vof_cache_backend_file_ce, SL("_useSafeKey"), 0, ZEND_ACC_PRIVATE TSRMLS_CC);
...
PHP_METHOD(Vof_Cache_Backend_File, __construct) {
	...
	if (!(zephir_array_isset_string(&options, SL("cacheDir")))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cache directory must be specified with the option cacheDir", "vof/cache/backend/file.zep", 79);
		return;
	}
	...
	if (zephir_array_isset_string_fetch(&safekey, &options, SL("safekey"), 0)) {
		if (((Z_TYPE_P(&safekey) == IS_TRUE || Z_TYPE_P(&safekey) == IS_FALSE) != 1)) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "safekey option should be a boolean.", "vof/cache/backend/file.zep", 84);
			return;
		}
	...
	if (zephir_array_isset_string_fetch(&prefix, &options, SL("prefix"), 0)) {
		...
		if (_1$$6) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "FileCache prefix should only use alphanumeric characters.", "vof/cache/backend/file.zep", 93);
			return;
		}
	}
	ZEPHIR_CALL_PARENT(NULL, vof_cache_backend_file_ce, getThis(), "__construct", &_5, 0, frontend, &options);
...
PHP_METHOD(Vof_Cache_Backend_File, get) {
	...
	if (!(zephir_array_isset_string_fetch(&cacheDir, &_2, SL("cacheDir"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/file.zep", 111);
		return;
	}
	...
	if ((zephir_file_exists(&cacheFile TSRMLS_CC) == SUCCESS) == 1) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "clearstatcache", NULL, 18, &__$true, &cacheFile);
		zephir_check_call_status();
		...
		if (ZEPHIR_LT_LONG(&_5$$4, (modifiedTime + ttl))) {
			...
			if (ZEPHIR_IS_FALSE_IDENTICAL(&cachedContent)) {
				...
				object_init_ex(&_6$$10, vof_cache_exception_ce);
				ZEPHIR_INIT_VAR(&_7$$10);
				ZEPHIR_CONCAT_SVS(&_7$$10, "Cache file ", &cacheFile, " could not be opened");
				ZEPHIR_CALL_METHOD(NULL, &_6$$10, "__construct", NULL, 19, &_7$$10);
				zephir_check_call_status();
				zephir_throw_exception_debug(&_6$$10, "vof/cache/backend/file.zep", 148 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
...
PHP_METHOD(Vof_Cache_Backend_File, save) {
	...
	if (!(zephir_is_true(&lastKey))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cache must be started first", "vof/cache/backend/file.zep", 186);
		return;
	}
	ZEPHIR_OBS_VAR(&frontend);
	zephir_read_property(&frontend, this_ptr, SL("_frontend"), PH_NOISY_CC);
	...
	zephir_read_property(&_2, this_ptr, SL("_options"), PH_NOISY_CC | PH_READONLY);
	if (!(zephir_array_isset_string_fetch(&cacheDir, &_2, SL("cacheDir"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/file.zep", 192);
		return;
	}
	ZEPHIR_INIT_VAR(&cacheFile);
	...
	if (ZEPHIR_IS_FALSE_IDENTICAL(&status)) {
		...
		object_init_ex(&_3$$11, vof_cache_exception_ce);
		ZEPHIR_INIT_VAR(&_4$$11);
		ZEPHIR_CONCAT_SVS(&_4$$11, "Cache file ", &cacheFile, " could not be written");
		ZEPHIR_CALL_METHOD(NULL, &_3$$11, "__construct", NULL, 19, &_4$$11);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_3$$11, "vof/cache/backend/file.zep", 215 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Vof_Cache_Backend_File, delete) {
	...
	if (!(zephir_array_isset_string_fetch(&cacheDir, &_0, SL("cacheDir"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/file.zep", 243);
		return;
	}
	...
	if ((zephir_file_exists(&cacheFile TSRMLS_CC) == SUCCESS)) {
		ZEPHIR_RETURN_CALL_FUNCTION("unlink", NULL, 20, &cacheFile);
		zephir_check_call_status();
		RETURN_MM();
	}
	RETURN_MM_BOOL(0);
...
PHP_METHOD(Vof_Cache_Backend_File, queryKeys) {
	...
	zephir_read_property(&_0, this_ptr, SL("_options"), PH_NOISY_CC | PH_READONLY);
	if (!(zephir_array_isset_string_fetch(&cacheDir, &_0, SL("cacheDir"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/file.zep", 270);
		return;
	}
	...
	ZEPHIR_CALL_METHOD(NULL, &_4, "__construct", NULL, 21, &cacheDir);
	zephir_check_call_status();
		...
		if (EXPECTED(ZEPHIR_IS_FALSE_IDENTICAL(&_5$$5))) {
			...
			if (!(ZEPHIR_IS_EMPTY(&prefix))) {
				if (zephir_start_with(&key, &prefixedKey, NULL)) {
					zephir_array_append(&keys, &key, PH_SEPARATE, "vof/cache/backend/file.zep", 285);
				}
			} else {
				zephir_array_append(&keys, &key, PH_SEPARATE, "vof/cache/backend/file.zep", 288);
			}
		}
	}
...
PHP_METHOD(Vof_Cache_Backend_File, exists) {
	...
	if (zephir_is_true(&lastKey)) {
		...
		zephir_array_fetch_string(&_2$$5, &_1$$5, SL("cacheDir"), PH_NOISY | PH_READONLY, "vof/cache/backend/file.zep", 315 TSRMLS_CC);
		ZEPHIR_INIT_VAR(&cacheFile);
...
PHP_METHOD(Vof_Cache_Backend_File, increment) {
	...
	zephir_update_property_zval(this_ptr, SL("_lastKey"), &prefixedKey);
	zephir_read_property(&_2, this_ptr, SL("_options"), PH_NOISY_CC | PH_READONLY);
	zephir_array_fetch_string(&_3, &_2, SL("cacheDir"), PH_NOISY | PH_READONLY, "vof/cache/backend/file.zep", 352 TSRMLS_CC);
	ZEPHIR_INIT_VAR(&cacheFile);
	...
	if ((zephir_file_exists(&cacheFile TSRMLS_CC) == SUCCESS)) {
		...
		if (ZEPHIR_LT_LONG(&_5$$3, (modifiedTime + zephir_get_numberval(&ttl)))) {
			...
			if (ZEPHIR_IS_FALSE_IDENTICAL(&cachedContent)) {
				...
				object_init_ex(&_6$$7, vof_cache_exception_ce);
				ZEPHIR_INIT_VAR(&_7$$7);
				ZEPHIR_CONCAT_SVS(&_7$$7, "Cache file ", &cacheFile, " could not be opened");
				ZEPHIR_CALL_METHOD(NULL, &_6$$7, "__construct", NULL, 19, &_7$$7);
				zephir_check_call_status();
				zephir_throw_exception_debug(&_6$$7, "vof/cache/backend/file.zep", 383 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
			if (zephir_is_numeric(&cachedContent)) {
				...
				if (ZEPHIR_IS_FALSE_IDENTICAL(&_8$$8)) {
					ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cache directory could not be written", "vof/cache/backend/file.zep", 390);
					return;
				}
				RETURN_CCTOR(&result);
			}
...
PHP_METHOD(Vof_Cache_Backend_File, decrement) {
	...
	zephir_update_property_zval(this_ptr, SL("_lastKey"), &prefixedKey);
	zephir_read_property(&_2, this_ptr, SL("_options"), PH_NOISY_CC | PH_READONLY);
	zephir_array_fetch_string(&_3, &_2, SL("cacheDir"), PH_NOISY | PH_READONLY, "vof/cache/backend/file.zep", 412 TSRMLS_CC);
	ZEPHIR_INIT_VAR(&cacheFile);
	ZEPHIR_CONCAT_VV(&cacheFile, &_3, &prefixedKey);
	if ((zephir_file_exists(&cacheFile TSRMLS_CC) == SUCCESS)) {
		ZEPHIR_OBS_VAR(&lifetime);
		...
		if (ZEPHIR_LT_LONG(&_6$$3, (modifiedTime + zephir_get_numberval(&ttl)))) {
			...
			if (ZEPHIR_IS_FALSE_IDENTICAL(&cachedContent)) {
				ZEPHIR_INIT_VAR(&_7$$7);
				object_init_ex(&_7$$7, vof_cache_exception_ce);
				ZEPHIR_INIT_VAR(&_8$$7);
				ZEPHIR_CONCAT_SVS(&_8$$7, "Cache file ", &cacheFile, " could not be opened");
				ZEPHIR_CALL_METHOD(NULL, &_7$$7, "__construct", NULL, 19, &_8$$7);
				zephir_check_call_status();
				zephir_throw_exception_debug(&_7$$7, "vof/cache/backend/file.zep", 441 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
			if (zephir_is_numeric(&cachedContent)) {
				...
				if (ZEPHIR_IS_FALSE_IDENTICAL(&_9$$8)) {
					ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cache directory can't be written", "vof/cache/backend/file.zep", 448);
					return;
				}
				RETURN_CCTOR(&result);
			}
...
PHP_METHOD(Vof_Cache_Backend_File, flush) {
	...
	zephir_read_property(&_0, this_ptr, SL("_options"), PH_NOISY_CC | PH_READONLY);
	if (!(zephir_array_isset_string_fetch(&cacheDir, &_0, SL("cacheDir"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/file.zep", 469);
		return;
	}
	ZEPHIR_INIT_VAR(&_2);
	object_init_ex(&_2, spl_ce_DirectoryIterator);
	ZEPHIR_CALL_METHOD(NULL, &_2, "__construct", NULL, 21, &cacheDir);
	zephir_check_call_status();
	...
	for (;_1->funcs->valid(_1 TSRMLS_CC) == SUCCESS && !EG(exception); _1->funcs->move_forward(_1 TSRMLS_CC)) {
		...
		if (EXPECTED(ZEPHIR_IS_TRUE(&_3$$4))) {
			...
			if (_4$$5) {
				ZEPHIR_CALL_FUNCTION(&_5$$6, "unlink", &_6, 20, &cacheFile);
				zephir_check_call_status();
