 ...
 *</code>
 */
ZEPHIR_INIT_CLASS(Vof_Cache_Backend_Mongo) {

	ZEPHIR_REGISTER_CLASS_EX(Vof\\Cache\\Backend, Mongo, vof, cache_backend_mongo, vof_cache_backend_ce, vof_cache_backend_mongo_method_entry, 0);

	zend_declare_property_null(vof_cache_backend_mongo_ce, SL("_collection"), ZEND_ACC_PROTECTED TSRMLS_CC);
 ...
 * @param array options
 */
PHP_METHOD(Vof_Cache_Backend_Mongo, __construct) {
	...
	if (!(zephir_array_isset_string(options, SL("mongo")))) {
		if (!(zephir_array_isset_string(options, SL("server")))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "The parameter 'server' is required", "vof/cache/backend/mongo.zep", 77);
			return;
		}
	}
	if (!(zephir_array_isset_string(options, SL("db")))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "The parameter 'db' is required", "vof/cache/backend/mongo.zep", 82);
		return;
	}
	if (!(zephir_array_isset_string(options, SL("collection")))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "The parameter 'collection' is required", "vof/cache/backend/mongo.zep", 86);
		return;
	}
	ZEPHIR_CALL_PARENT(NULL, vof_cache_backend_mongo_ce, getThis(), "__construct", &_0, 0, frontend, options);
 ...
 */
PHP_METHOD(Vof_Cache_Backend_Mongo, _getCollection) {
	...
	if (Z_TYPE_P(&mongoCollection) != IS_OBJECT) {
		...
		if (zephir_array_isset_string_fetch(&mongo, &options, SL("mongo"), 0)) {
			if (Z_TYPE_P(&mongo) != IS_OBJECT) {
				ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "The 'mongo' parameter must be a valid Mongo instance", "vof/cache/backend/mongo.zep", 111);
				return;
			}
		} else {
			ZEPHIR_OBS_VAR(&server);
			zephir_array_fetch_string(&server, &options, SL("server"), PH_NOISY, "vof/cache/backend/mongo.zep", 119 TSRMLS_CC);
			_0$$6 = !zephir_is_true(&server);
			if (!(_0$$6)) {
				_0$$6 = Z_TYPE_P(&server) != IS_STRING;
			}
			if (_0$$6) {
				ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "The backend requires a valid MongoDB connection string", "vof/cache/backend/mongo.zep", 121);
				return;
			}
		...
		zephir_array_fetch_string(&database, &options, SL("db"), PH_NOISY, "vof/cache/backend/mongo.zep", 130 TSRMLS_CC);
		_1$$3 = !zephir_is_true(&database);
		if (!(_1$$3)) {
			_1$$3 = Z_TYPE_P(&database) != IS_STRING;
		}
		if (_1$$3) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "The backend requires a valid MongoDB db", "vof/cache/backend/mongo.zep", 132);
			return;
		}
		ZEPHIR_OBS_VAR(&collection);
		zephir_array_fetch_string(&collection, &options, SL("collection"), PH_NOISY, "vof/cache/backend/mongo.zep", 138 TSRMLS_CC);
		_2$$3 = !zephir_is_true(&collection);
		if (!(_2$$3)) {
			_2$$3 = Z_TYPE_P(&collection) != IS_STRING;
		}
		if (_2$$3) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "The backend requires a valid MongoDB collection", "vof/cache/backend/mongo.zep", 140);
			return;
		}
 ...
 */
PHP_METHOD(Vof_Cache_Backend_Mongo, get) {
	...
	ZEPHIR_CALL_METHOD(&_3, this_ptr, "_getcollection", NULL, 22);
	zephir_check_call_status();
	...
	if (Z_TYPE_P(&document) == IS_ARRAY) {
		...
		if (zephir_array_isset_string_fetch(&cachedContent, &document, SL("data"), 0)) {
			...
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "The cache is corrupt", "vof/cache/backend/mongo.zep", 176);
			return;
		}
	}
	RETURN_MM_NULL();
 ...
 */
PHP_METHOD(Vof_Cache_Backend_Mongo, save) {
	...
	if (!(zephir_is_true(&lastkey))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cache must be started first", "vof/cache/backend/mongo.zep", 208);
		return;
	}
	ZEPHIR_OBS_VAR(&frontend);
	zephir_read_property(&frontend, this_ptr, SL("_frontend"), PH_NOISY_CC);
	...
	ZEPHIR_CALL_METHOD(&collection, this_ptr, "_getcollection", NULL, 22);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_1);
	zephir_time(&_1);
	...
	if (Z_TYPE_P(&document) == IS_ARRAY) {
		...
		zephir_array_fetch_string(&_3$$14, &document, SL("_id"), PH_NOISY, "vof/cache/backend/mongo.zep", 243 TSRMLS_CC);
	...
	if (!(zephir_is_true(&success))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Failed storing data in mongodb", "vof/cache/backend/mongo.zep", 252);
		return;
	}
 ...
 * @return boolean
 */
PHP_METHOD(Vof_Cache_Backend_Mongo, delete) {
	...
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "_getcollection", NULL, 22);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_1);
	zephir_create_array(&_1, 1, 0 TSRMLS_CC);
	...
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_4, "rand", NULL, 23);
	zephir_check_call_status();
 ...
 * </code>
 */
PHP_METHOD(Vof_Cache_Backend_Mongo, queryKeys) {
	...
	for (;_5->funcs->valid(_5 TSRMLS_CC) == SUCCESS && !EG(exception); _5->funcs->move_forward(_5 TSRMLS_CC)) {
		...
		zephir_is_iterable(&item, 0, "vof/cache/backend/mongo.zep", 317);
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&item), _7$$4, _8$$4, _6$$4)
		{
			...
			if (ZEPHIR_IS_STRING(&key, "key")) {
				zephir_array_append(&keys, &value, PH_SEPARATE, "vof/cache/backend/mongo.zep", 314);
			}
 ...
 * @param int lifetime
 */
PHP_METHOD(Vof_Cache_Backend_Mongo, exists) {
	...
	if (zephir_is_true(&lastKey)) {
		ZEPHIR_CALL_METHOD(&_1$$5, this_ptr, "_getcollection", NULL, 22);
		zephir_check_call_status();
 ...
 */
PHP_METHOD(Vof_Cache_Backend_Mongo, gc) {
 ...
 * @param int|string keyName
 */
PHP_METHOD(Vof_Cache_Backend_Mongo, increment) {
	...
	ZEPHIR_CONCAT_VV(&prefixedKey, &_0, keyName);
	zephir_update_property_zval(this_ptr, SL("_lastKey"), &prefixedKey);
	ZEPHIR_CALL_METHOD(&_1, this_ptr, "_getcollection", NULL, 22);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_2);
	zephir_create_array(&_2, 1, 0 TSRMLS_CC);
	...
	if (!(zephir_array_isset_string_fetch(&modifiedTime, &document, SL("time"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "The cache is corrupted", "vof/cache/backend/mongo.zep", 370);
		return;
	}
	ZEPHIR_INIT_VAR(&_3);
	zephir_time(&_3);
	if (ZEPHIR_LT(&_3, &modifiedTime)) {
		ZEPHIR_OBS_VAR(&cachedContent);
		if (!(zephir_array_isset_string_fetch(&cachedContent, &document, SL("data"), 0))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "The cache is corrupted", "vof/cache/backend/mongo.zep", 379);
			return;
		}
		if (zephir_is_numeric(&cachedContent)) {
 ...
 * @param int|string $keyName
 */
PHP_METHOD(Vof_Cache_Backend_Mongo, decrement) {
	...
	ZEPHIR_OBS_VAR(&modifiedTime);
	if (!(zephir_array_isset_string_fetch(&modifiedTime, &document, SL("time"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "The cache is corrupted", "vof/cache/backend/mongo.zep", 407);
		return;
	}
	ZEPHIR_INIT_VAR(&_3);
	zephir_time(&_3);
	if (ZEPHIR_LT(&_3, &modifiedTime)) {
		ZEPHIR_OBS_VAR(&cachedContent);
		if (!(zephir_array_isset_string_fetch(&cachedContent, &document, SL("data"), 0))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "The cache is corrupted", "vof/cache/backend/mongo.zep", 416);
			return;
		}
		if (zephir_is_numeric(&cachedContent)) {
 ...
 */
PHP_METHOD(Vof_Cache_Backend_Mongo, flush) {
