...
ZEPHIR_INIT_CLASS(Vof_Cache_Backend_Redis) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Vof\\Cache\\Backend, Redis, vof, cache_backend_redis, vof_cache_backend_ce, vof_cache_backend_redis_method_entry, 0);

	zend_declare_property_null(vof_cache_backend_redis_ce, SL("_redis"), ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Vof_Cache_Backend_Redis, __construct) {
	...
	ZEPHIR_CALL_PARENT(NULL, vof_cache_backend_redis_ce, getThis(), "__construct", &_5, 0, frontend, options);
...
PHP_METHOD(Vof_Cache_Backend_Redis, _connect) {
	...
	if (_1) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/redis.zep", 118);
		return;
	}
	...
	if (!(zephir_is_true(&success))) {
		ZEPHIR_INIT_VAR(&_2$$6);
		object_init_ex(&_2$$6, vof_cache_exception_ce);
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_2$$6, "vof/cache/backend/redis.zep", 128 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_OBS_VAR(&auth);
	...
	if (_4) {
		zephir_array_fetch_string(&_5, &options, SL("auth"), PH_NOISY | PH_READONLY, "vof/cache/backend/redis.zep", 131 TSRMLS_CC);
	...
	if (_4) {
		...
		zephir_check_call_status();
		if (!(zephir_is_true(&success))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Failed to authenticate with the Redisd server", "vof/cache/backend/redis.zep", 135);
			return;
		}
	}
	...
	if (_6) {
		...
		zephir_check_call_status();
		if (!(zephir_is_true(&success))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Redis server selected database failed", "vof/cache/backend/redis.zep", 143);
			return;
		}
	}
...
PHP_METHOD(Vof_Cache_Backend_Redis, save) {
	...
	if (!(zephir_is_true(&lastKey))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "The cache must be started first", "vof/cache/backend/redis.zep", 210);
		return;
	}
	ZEPHIR_OBS_VAR(&frontend);
	zephir_read_property(&frontend, this_ptr, SL("_frontend"), PH_NOISY_CC);
	...
	zephir_check_call_status();
	if (!(zephir_is_true(&success))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Failed storing the data in redis", "vof/cache/backend/redis.zep", 254);
		return;
	}
	...
	if (!(zephir_array_isset_string_fetch(&specialKey, &options, SL("statsKey"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/redis.zep", 265);
		return;
	}
...
PHP_METHOD(Vof_Cache_Backend_Redis, delete) {
	...
	ZEPHIR_OBS_VAR(&specialKey);
	if (!(zephir_array_isset_string_fetch(&specialKey, &options, SL("statsKey"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/redis.zep", 308);
		return;
	}
	if (!ZEPHIR_IS_STRING(&specialKey, "")) {
...
PHP_METHOD(Vof_Cache_Backend_Redis, queryKeys) {
	...
	ZEPHIR_OBS_VAR(&specialKey);
	if (!(zephir_array_isset_string_fetch(&specialKey, &options, SL("statsKey"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/redis.zep", 345);
		return;
	}
	if (ZEPHIR_IS_STRING(&specialKey, "")) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cached keys need to be enabled to use this function (options['statsKey'] == '_PHCR')!", "vof/cache/backend/redis.zep", 349);
		return;
	}
	...
	zephir_is_iterable(&keys, 1, "vof/cache/backend/redis.zep", 366);
...
PHP_METHOD(Vof_Cache_Backend_Redis, flush) {
	...
	ZEPHIR_OBS_VAR(&specialKey);
	if (!(zephir_array_isset_string_fetch(&specialKey, &options, SL("statsKey"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/redis.zep", 463);
		return;
	}
	ZEPHIR_OBS_VAR(&redis);
	zephir_read_property(&redis, this_ptr, SL("_redis"), PH_NOISY_CC);
	...
	if (ZEPHIR_IS_STRING(&specialKey, "")) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cached keys need to be enabled to use this function (options['statsKey'] == '_PHCR')!", "vof/cache/backend/redis.zep", 474);
		return;
	}
	ZEPHIR_CALL_METHOD(&keys, &redis, "smembers", NULL, 0, &specialKey);
	zephir_check_call_status();
	if (Z_TYPE_P(&keys) == IS_ARRAY) {
		zephir_is_iterable(&keys, 0, "vof/cache/backend/redis.zep", 484);
