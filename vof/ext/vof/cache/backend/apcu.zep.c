...
ZEPHIR_INIT_CLASS(Vof_Cache_Backend_Apcu) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Vof\\Cache\\Backend, Apcu, vof, cache_backend_apcu, vof_cache_backend_ce, vof_cache_backend_apcu_method_entry, 0);
...
PHP_METHOD(Vof_Cache_Backend_Apcu, save) {
	...
	if (!(zephir_is_true(&lastKey))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cache must be started first", "vof/cache/backend/apcu.zep", 95);
		return;
	}
	...
	if (!(zephir_is_true(&success))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Failed storing data in APCu", "vof/cache/backend/apcu.zep", 132);
		return;
	}
...
PHP_METHOD(Vof_Cache_Backend_Apcu, queryKeys) {
	...
	for (;_5->funcs->valid(_5 TSRMLS_CC) == SUCCESS && !EG(exception); _5->funcs->move_forward(_5 TSRMLS_CC)) {
		...
		zephir_array_append(&keys, &_8$$8, PH_SEPARATE, "vof/cache/backend/apcu.zep", 222);
	}
...
PHP_METHOD(Vof_Cache_Backend_Apcu, flush) {
	...
	for (;_5->funcs->valid(_5 TSRMLS_CC) == SUCCESS && !EG(exception); _5->funcs->move_forward(_5 TSRMLS_CC)) {
		...
		zephir_array_fetch_string(&_6$$6, &item, SL("key"), PH_NOISY | PH_READONLY, "vof/cache/backend/apcu.zep", 283 TSRMLS_CC);
