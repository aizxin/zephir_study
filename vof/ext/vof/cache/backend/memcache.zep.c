...
ZEPHIR_INIT_CLASS(Vof_Cache_Backend_Memcache) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Vof\\Cache\\Backend, Memcache, vof, cache_backend_memcache, vof_cache_backend_ce, vof_cache_backend_memcache_method_entry, 0);

	zend_declare_property_null(vof_cache_backend_memcache_ce, SL("_memcache"), ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Vof_Cache_Backend_Memcache, __construct) {
	...
	ZEPHIR_CALL_PARENT(NULL, vof_cache_backend_memcache_ce, getThis(), "__construct", &_3, 0, frontend, options);
...
PHP_METHOD(Vof_Cache_Backend_Memcache, _connect) {
	...
	if (_1) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/memcache.zep", 109);
		return;
	}
	...
	if (!(zephir_is_true(&success))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cannot connect to Memcached server", "vof/cache/backend/memcache.zep", 119);
		return;
	}
...
PHP_METHOD(Vof_Cache_Backend_Memcache, save) {
	...
	if (!(zephir_is_true(&lastKey))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cache must be started first", "vof/cache/backend/memcache.zep", 194);
		return;
	}
	...
	zephir_check_call_status();
	if (!(zephir_is_true(&success))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Failed storing data in memcached", "vof/cache/backend/memcache.zep", 241);
		return;
	}
	ZEPHIR_OBS_VAR(&options);
	zephir_read_property(&options, this_ptr, SL("_options"), PH_NOISY_CC);
	...
	if (!(zephir_array_isset_string_fetch(&specialKey, &options, SL("statsKey"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/memcache.zep", 247);
		return;
	}
...
PHP_METHOD(Vof_Cache_Backend_Memcache, delete) {
	...
	ZEPHIR_OBS_VAR(&specialKey);
	if (!(zephir_array_isset_string_fetch(&specialKey, &options, SL("statsKey"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/memcache.zep", 300);
		return;
	}
	if (!ZEPHIR_IS_STRING(&specialKey, "")) {
		ZEPHIR_CALL_METHOD(&keys, &memcache, "get", NULL, 0, &specialKey);
...
PHP_METHOD(Vof_Cache_Backend_Memcache, queryKeys) {
	...
	ZEPHIR_OBS_VAR(&specialKey);
	if (!(zephir_array_isset_string_fetch(&specialKey, &options, SL("statsKey"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/memcache.zep", 343);
		return;
	}
	if (ZEPHIR_IS_STRING(&specialKey, "")) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cached keys need to be enabled to use this function (options['statsKey'] == '_PHCM')!", "vof/cache/backend/memcache.zep", 347);
		return;
	}
	ZEPHIR_CALL_METHOD(&keys, &memcache, "get", NULL, 0, &specialKey);
	zephir_check_call_status();
	...
	zephir_is_iterable(&keys, 1, "vof/cache/backend/memcache.zep", 365);
...
PHP_METHOD(Vof_Cache_Backend_Memcache, flush) {
	...
	ZEPHIR_OBS_VAR(&specialKey);
	if (!(zephir_array_isset_string_fetch(&specialKey, &options, SL("statsKey"), 0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Unexpected inconsistency in options", "vof/cache/backend/memcache.zep", 473);
		return;
	}
	if (ZEPHIR_IS_STRING(&specialKey, "")) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cached keys need to be enabled to use this function (options['statsKey'] == '_PHCM')!", "vof/cache/backend/memcache.zep", 477);
		return;
	}
	ZEPHIR_CALL_METHOD(&keys, &memcache, "get", NULL, 0, &specialKey);
	zephir_check_call_status();
	...
	ZEPHIR_INIT_VAR(&_0);
	zephir_is_iterable(&keys, 0, "vof/cache/backend/memcache.zep", 492);
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&keys), _2, _3, _1)
	{
		ZEPHIR_INIT_NVAR(&key);
		if (_3 != NULL) { 
