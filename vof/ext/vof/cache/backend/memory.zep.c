...
ZEPHIR_INIT_CLASS(Vof_Cache_Backend_Memory) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Vof\\Cache\\Backend, Memory, vof, cache_backend_memory, vof_cache_backend_ce, vof_cache_backend_memory_method_entry, 0);

	zend_declare_property_null(vof_cache_backend_memory_ce, SL("_data"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_class_implements(vof_cache_backend_memory_ce TSRMLS_CC, 1, zend_ce_serializable);
	return SUCCESS;
...
PHP_METHOD(Vof_Cache_Backend_Memory, save) {
	...
	if (!(zephir_is_true(&lastKey))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cache must be started first", "vof/cache/backend/memory.zep", 95);
		return;
	}
...
PHP_METHOD(Vof_Cache_Backend_Memory, queryKeys) {
	...
	zephir_is_iterable(&keys, 1, "vof/cache/backend/memory.zep", 177);
...
PHP_METHOD(Vof_Cache_Backend_Memory, unserialize) {
	...
	ZEPHIR_CALL_FUNCTION(&unserialized, "unserialize", NULL, 11, data);
	zephir_check_call_status();
	if (Z_TYPE_P(&unserialized) != IS_ARRAY) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(zend_exception_get_default(TSRMLS_C), "Unserialized data must be an array", "vof/cache/backend/memory.zep", 295);
		return;
	}
	zephir_array_fetch_string(&_0, &unserialized, SL("frontend"), PH_NOISY | PH_READONLY, "vof/cache/backend/memory.zep", 298 TSRMLS_CC);
