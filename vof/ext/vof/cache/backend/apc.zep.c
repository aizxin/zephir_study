...
ZEPHIR_INIT_CLASS(Vof_Cache_Backend_Apc) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Vof\\Cache\\Backend, Apc, vof, cache_backend_apc, vof_cache_backend_ce, vof_cache_backend_apc_method_entry, 0);
...
PHP_METHOD(Vof_Cache_Backend_Apc, get) {
	...
	ZEPHIR_CALL_FUNCTION(&cachedContent, "apc_fetch", NULL, 12, &prefixedKey);
...
PHP_METHOD(Vof_Cache_Backend_Apc, save) {
	...
	if (!(zephir_is_true(&lastKey))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Cache must be started first", "vof/cache/backend/apc.zep", 98);
		return;
	}
	...
	ZEPHIR_CALL_FUNCTION(&success, "apc_store", NULL, 13, &lastKey, &preparedContent, &ttl);
	zephir_check_call_status();
	if (!(zephir_is_true(&success))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(vof_cache_exception_ce, "Failed storing data in apc", "vof/cache/backend/apc.zep", 135);
		return;
	}
...
PHP_METHOD(Vof_Cache_Backend_Apc, increment) {
	...
	if ((zephir_function_exists_ex(SL("apc_inc") TSRMLS_CC) == SUCCESS)) {
		...
		ZEPHIR_CALL_FUNCTION(&result, "apc_inc", NULL, 14, &prefixedKey, &_1$$3);
		zephir_check_call_status();
		RETURN_CCTOR(&result);
	} else {
		ZEPHIR_CALL_FUNCTION(&cachedContent, "apc_fetch", NULL, 12, &prefixedKey);
...
PHP_METHOD(Vof_Cache_Backend_Apc, decrement) {
	...
	if ((zephir_function_exists_ex(SL("apc_dec") TSRMLS_CC) == SUCCESS)) {
		ZVAL_LONG(&_1$$3, value);
		ZEPHIR_RETURN_CALL_FUNCTION("apc_dec", NULL, 15, &lastKey, &_1$$3);
		zephir_check_call_status();
		RETURN_MM();
	} else {
		ZEPHIR_CALL_FUNCTION(&cachedContent, "apc_fetch", NULL, 12, &lastKey);
		zephir_check_call_status();
		if (zephir_is_numeric(&cachedContent)) {
...
PHP_METHOD(Vof_Cache_Backend_Apc, delete) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("apc_delete", NULL, 16, &_1);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Vof_Cache_Backend_Apc, queryKeys) {
	...
	for (;_2->funcs->valid(_2 TSRMLS_CC) == SUCCESS && !EG(exception); _2->funcs->move_forward(_2 TSRMLS_CC)) {
		...
		zephir_array_append(&keys, &_4$$5, PH_SEPARATE, "vof/cache/backend/apc.zep", 240);
	}
...
PHP_METHOD(Vof_Cache_Backend_Apc, exists) {
	...
	if (zephir_is_true(&lastKey)) {
		ZEPHIR_CALL_FUNCTION(&_1$$5, "apc_exists", NULL, 17, &lastKey);
		zephir_check_call_status();
...
PHP_METHOD(Vof_Cache_Backend_Apc, flush) {
	...
	for (;_1->funcs->valid(_1 TSRMLS_CC) == SUCCESS && !EG(exception); _1->funcs->move_forward(_1 TSRMLS_CC)) {
		...
		zephir_array_fetch_string(&_4$$3, &item, SL("key"), PH_NOISY | PH_READONLY, "vof/cache/backend/apc.zep", 292 TSRMLS_CC);
		ZEPHIR_CALL_FUNCTION(NULL, "apc_delete", &_5, 16, &_4$$3);
		zephir_check_call_status();
	}
