...
use Vof\Db;
use Vof\Db\ColumnInterface;
use Vof\Events\EventsAwareInterface;
use Vof\Events\ManagerInterface;

/**
 * Vof\Db\Adapter
 *
 * Base class for Vof\Db adapters
 */
abstract class Adapter implements AdapterInterface, EventsAwareInterface
{
	...
	public function __construct(array! descriptor)
	{
		...
		if !fetch dialectClass, descriptor["dialectClass"] {
			let dialectClass = "Vof\\db\\dialect\\" . this->_dialectType;
		}
	 ...
	 * $robot = $connection->fetchOne("SELECT * FROM robots", \Vof\Db::FETCH_ASSOC);
	 * print_r($robot);
	 *</code>
	 */
	...
	public function describeIndexes(string! table, schema = null) -> <Index[]>
	{
		...
		for name, indexColumns in indexes {
			...
			/**
			 * Every index is abstracted using a Vof\Db\Index instance
			 */
