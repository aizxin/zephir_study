...
/*
 ...
 | Version 1.4.5                                                      |
 +------------------------------------------------------------------------+
 | Copyright (c) 2011-2017 Phalcon Team (https://medoo.in)          |
 +------------------------------------------------------------------------+
...
class Vdo
{
	...
	{
		var tmpArray,tmpArray2,group,map_key,columns,where_clause="",where_keys,where_OR,where_AND,single_condition,condition,value,match1,mode,mode_array=[];
		let this->map = [];
		if (is_array(where))
		{
                ...
                if (match1[ "join" ] !== "" && match1[ "table" ] !== "")
                {
                    ...
                    if (is_array(relation))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                let joins[] = (strpos(key, ".") > 0 ? this->columnQuote(key) : table . ".`" . key . "`" ) . " = " . this->tableQuote(isset(match1[ "alias" ]) ? match1[ "alias" ] : match1[ "table" ]) . ".`" . value . "`";
                            }
